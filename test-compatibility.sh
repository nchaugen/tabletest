...
#   TABLETEST_VERSION="0.5.7-SNAPSHOT"  # default TableTest version (can be overridden via env)
# Usage examples:
...
#   TABLETEST_VERSION="0.5.7-SNAPSHOT" ./test-compatibility.sh
...
if [ -z "$JUNIT_VERSIONS" ]; then JUNIT_VERSIONS="6.0.1 5.14.1 5.13.4 5.12.2"; fi
if [ -z "$QUARKUS_VERSIONS" ]; then QUARKUS_VERSIONS="3.30.2 3.24.0 3.23.4 3.21.2"; fi
if [ -z "$SPRINGBOOT_VERSIONS" ]; then SPRINGBOOT_VERSIONS="4.0.0 3.5.7"; fi
if [ -z "$TABLETEST_VERSION" ]; then TABLETEST_VERSION="0.5.7-SNAPSHOT"; fi
